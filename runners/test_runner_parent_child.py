...
from z3_core.vector_parent_child import build_parent_child_index, get_parent_child_retriever
...
def ensure_vector_store(config) -> bool:
    ...
    try:
        # Get parent-child config parameters
        parent_headers = getattr(config, 'parent_headers_to_split', ["##"])
        child_chunk_size = getattr(config, 'child_chunk_size', 450)
        child_chunk_overlap = getattr(config, 'child_chunk_overlap', 50)
        parent_max_tokens = getattr(config, 'parent_max_tokens', 1500)
        ...
        print(f"✓ Parent-child vector store built successfully")
        return True
    except Exception as e:
        print(f"✗ Failed to build parent-child vector store: {e}")
        return False
...
def main():
    ...
    print("\n" + "="*60)
    print("RAG EVALUATION TEST RUNNER - PARENT-CHILD MARKDOWN")
    print("="*60 + "\n")

    # Load domain config (check experiments_phase8e/ first, then root configs/)
    try:
        try:
            config = load_domain_config(args.domain, config_dir=Path("configs/experiments_phase8e"))
            print(f"✓ Loaded domain config: {args.domain} (from experiments_phase8e/)")
